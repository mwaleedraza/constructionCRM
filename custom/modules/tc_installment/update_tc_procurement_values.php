...
class update_tc_procurement_values{
	public function update_method($bean, $event, $arguments){
		...
		if($bean->installment_paid_c==1 && $bean->installment_amount_c !=$bean->fetched_row['installment_amount_c'] ){
			$GLOBALS['log']->fatal("reached");
			$GLOBALS['log']->fatal("installment amount".$bean->installment_amount_c);
				$GLOBALS['log']->fatal("amount paid".$tc_procurementBean->amount_paid_c);
					$GLOBALS['log']->fatal("old". $bean->fetched_row['installment_amount_c']);
			if($bean->fetched_row['installment_paid_c']=='1'){
